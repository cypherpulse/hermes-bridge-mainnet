...
- [CCTP v2 Integration](#cctp-v2-integration)
  - [Fast vs Standard Transfer](#fast-vs-standard-transfer)
  - [Fee Model](#fee-model)
...
Hermes bridges USDC using Circle's **canonical CCTP v2** — not the legacy v1 protocol. The EVM-to-EVM / EVM-to-Ethereum leg runs through [`@circle-fin/bridge-kit`](https://developers.circle.com/cctp), which wraps `@circle-fin/provider-cctp-v2` and issues real `TokenMessengerV2.depositForBurn` calls (`destinationCaller`, `maxFee`, `minFinalityThreshold`) under the hood. The Ethereum → Stacks leg uses [xReserve](https://developers.circle.com/xreserve), Circle's CCTP v2-based wrapper for non-EVM destinations, via `depositToRemote`.

Because xReserve currently only accepts deposits from Ethereum (not other EVM chains directly), every non-Ethereum source still routes **Source Chain → Ethereum (CCTP v2) → Stacks (xReserve)** — see [Bridge Timeline](#bridge-timeline). This hub-and-spoke shape is a protocol constraint, not a Hermes limitation.

### Fast vs Standard Transfer

Every bridge defaults to **Fast Transfer** and automatically falls back to **Standard Transfer** if a live fee quote isn't available for the route:

| Mode | `minFinalityThreshold` | Speed | Cost |
|------|------------------------|-------|------|
| Fast | 1000 | Seconds to a few minutes | Circle's Fast Transfer fee (0-13 bps, varies by source chain) |
| Standard | 2000 | Full chain finality (minutes for L2s, longer for L1s) | Free |

Before submitting, Hermes fetches a live quote from Circle's `GET /v2/burn/USDC/fees/{sourceDomain}/{destDomain}` and passes a buffered `maxFee` so the fast-transfer transaction doesn't revert if the quoted fee drifts. Attestation status is read from Circle's consolidated v2 endpoint, `GET /v2/messages/{sourceDomainId}?transactionHash=...`, instead of polling by elapsed time. See [`src/lib/cctp-fees.ts`](src/lib/cctp-fees.ts) and [`src/lib/cctp-attestation.ts`](src/lib/cctp-attestation.ts).

### Fee Model

- **Circle's fee** is Fast-Transfer-only, quoted live in basis points per route, and shown to the user before they submit.
- **Hermes's own fee** is charged only on the EVM-to-EVM / EVM-to-Ethereum leg, via Bridge Kit's native `customFee` (charged on top of the transfer amount, auto-split 10% to Circle / 90% to Hermes). It's computed as basis points of the amount, clamped to a floor and cap, all env-configurable:

  ```
  VITE_PROTOCOL_FEE_BPS=6
  VITE_PROTOCOL_FEE_MIN_USDC=0.02
  VITE_PROTOCOL_FEE_MAX_USDC=5
  ```

- The Ethereum → Stacks (xReserve) leg does not currently charge a Hermes fee — xReserve's `depositToRemote` ABI has no fee-recipient parameter.

### Supported Domains

CCTP domain IDs used for fee quotes and attestation lookups (stable across mainnet/testnet):

| Chain | Domain | Chain | Domain |
|-------|--------|-------|--------|
| Ethereum | 0 | Polygon PoS | 7 |
| Avalanche | 1 | Unichain | 10 |
| OP (Optimism) | 2 | Linea | 11 |
| Arbitrum | 3 | World Chain | 14 |
| Base | 6 | Arc (testnet only) | 26 |
| **Stacks (xReserve remote)** | **10003** | | |
